...
def read_file():
    f = open(os.getcwd() + '/../WaldoA2C_1_LTL_traces.json', 'r')
    lines = f.readlines()
    f.close()
    return lines
            ...
            for k in converted:
                for l in k:
                    if l['is_dummy'] and j['is_dummy']:
                        l['No'] = j['No']
                    elif l['waldo'] == j['waldo'] and l['r1'] == j['r1'] and \
                            l['r2'] == j['r2'] and \
                            l['r3'] == j['r3'] and l['r4'] == j['r4']:
                        l['No'] = j['No']
    return converted
...
def generate_prism_model(state_list):
    ...
    for state in state_list:
        data += '\t[] '
        data += '(waldo = ' + str(state['waldo']).lower() + ') & (r1 = ' + str(state['r1']).lower() + \
                ') & (r2 = ' + str(state['r2']).lower() + \
                ') & (r3 = ' + str(state['r3']).lower() + ') & (r4 = ' + str(state['r4']).lower() + ') -> '
        if len(state['next']) > 0:
            for next in state['next']:
                data += str(next['probability']) + ':(waldo\' = ' + str(next['waldo']).lower() + ') & (r1\' = ' + str(next['r1']).lower() + \
                        ') & (r2\' = ' + str(next['r2']).lower() + \
                        ') & (r3\' = ' + str(next['r3']).lower() + ') & (r4\' = ' + str(next['r4']).lower() + ') + '
            data = data[:-3]
            data += ';\n'
        else:
            data += '1:(waldo\' = ' + str(state['waldo']).lower() + ') & (r1\' = ' + str(state['r1']).lower() +  \
                    ') & (r2\' = ' + str(state['r2']).lower() + \
                    ') & (r3\' = ' + str(state['r3']).lower() + ') & (r4\' = ' + str(state['r4']).lower() + ')'
            data += ';\n'
    data += 'endmodule\n'
    ...
    for state in state_list:
        if not state['is_dummy']:
            data += 'label \"s' + str(state['No']) + '\" = ' + '(waldo = ' + str(state['waldo']).lower() + ') & (r1 = ' + str(state['r1']).lower() + \
                ') & (r2 = ' + str(state['r2']).lower() + \
                ') & (r3 = ' + str(state['r3']).lower() + ') & (r4 = ' + str(state['r4']).lower() + ');\n'
    for state in state_list:
        if state['is_dummy']:
            data += 'label \"dummy' + str(state['No']) + '\" = ' + '(waldo = ' + str(state['waldo']).lower() + ') & (r1 = ' + str(state['r1']).lower() + \
                ') & (r2 = ' + str(state['r2']).lower() + \
                ') & (r3 = ' + str(state['r3']).lower() + ') & (r4 = ' + str(state['r4']).lower() + ');\n'
    data += '\n'
    data += 'rewards "step"\n'
