...
from SolvingSat.solving_sat_env_v9 import SolvingSatEnv_v9
...
env = SolvingSatEnv_v9()

# We get the shape of a state and the actions space size
state_size = env.observation_space.shape[0]
action_size = env.action_space[0].n * env.action_space[1].n
...
def test():
    ...
    for i in range(max_episode):
        ...
        for j in range(max_iteration_ep):
            ...
            env.take_env(action=action)
            if done:
                break
            if steps == max_iteration_ep - 1:
                total_steps += max_iteration_ep
                solve += 1
                break
