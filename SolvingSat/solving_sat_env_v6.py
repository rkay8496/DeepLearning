    ...
    def step(self, action):
        ...
        for key in self.formula.keys():
            ...
            if evaluation:
                ...
                reward += -10
            self.formula[key] = evaluation

        count = len(dict(filter(lambda elem: elem[1] == True, self.formula.items())).keys())
        if count == len(self.formula.keys()):
            reward += 50
            done = True
        else:
            reward += -500
