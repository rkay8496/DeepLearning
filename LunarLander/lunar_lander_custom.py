...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        self.moon = self.world.CreateStaticBody(
            shapes=Box2D.b2.edgeShape(vertices=[(0, 0), (W, 0)])
        )
        self.sky_polys = []
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                ...
                angle=(i * 0.05),
                fixtures=Box2D.b2.fixtureDef(
                    shape=Box2D.b2.polygonShape(box=(LEG_W / SCALE, LEG_H / SCALE)),
                    density=1.0,
                    restitution=0.0,
                    categoryBits=0x0020,
                    maskBits=0x001,
    ...
    def _create_particle(self, mass, x, y, ttl):
        p = self.world.CreateDynamicBody(
            ...
            angle=0.0,
            fixtures=Box2D.b2.fixtureDef(
                shape=Box2D.b2.circleShape(radius=2 / SCALE, pos=(0, 0)),
                density=mass,
                friction=0.1,
                categoryBits=0x0100,
                maskBits=0x001,  # collide only with ground
