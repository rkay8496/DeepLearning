...
class ActorCritic(gym.Env):
    ...
    def __init__(self, render_mode=None, size=5):
        ...
        self.observation_space = Dict(
            {
                'speed': Box(low=0, high=100, shape=(1,), dtype=np.float32),
                'mode': Discrete(5, start=1),
                'timegap': Box(low=0, high=np.inf, shape=(1,), dtype=np.float32),
                'power': Discrete(2),
                'distance': Box(low=0, high=100, shape=(1,), dtype=np.float32),
            }
        )
    ...
    def reset(self):
        ...
        self.traces['speed'].append((len(self.traces['speed']), self.observation['speed'][0]))
        self.traces['mode'].append((len(self.traces['mode']), self.observation['mode']))
        self.traces['timegap'].append((len(self.traces['timegap']), self.observation['timegap'][0]))
        self.traces['power'].append((len(self.traces['power']), self.observation['power']))
        self.traces['distance'].append((len(self.traces['distance']), self.observation['distance'][0]))
        print(flatten(Discrete, self.observation))
        exit()
